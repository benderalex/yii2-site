 ...
 *
 */
class ManyHasManyBehavior extends Behavior
{
    /**
     * List of relations.
     * @var array
            ...
            if (!empty($relation->via) && $relation->multiple) {
                ...
                try {
                    if (!empty($this->_values[$attributeName])) {
                        $oldValues = $relation->select($relationModel->getPrimaryKey())->column();
                        ...
                        if (count($insert)) {
                            Yii::$app->db->createCommand()
                                ->batchInsert($junctionTable, [$relatedColumn, $ownerColumn], $insert)
                                ->execute();
                        }
                        Yii::$app->db->createCommand()
                            ->delete($junctionTable, $ownerColumn.'="'.$ownerPk.'" AND '.$relatedColumn.' NOT IN ('.implode(',',$newValues).')')
                            ->execute();
                    } else {
                        Yii::$app->db->createCommand()
